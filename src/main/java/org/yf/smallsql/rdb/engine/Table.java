...
import org.yf.smallsql.rdb.engine.store.StorePageLink;
import org.yf.smallsql.rdb.engine.store.TableStorePage;
import org.yf.smallsql.rdb.engine.store.TableStorePageInsert;
import org.yf.smallsql.rdb.engine.table.Columns;
import org.yf.smallsql.rdb.engine.table.ForeignKey;
import org.yf.smallsql.rdb.engine.table.ForeignKeys;
import org.yf.smallsql.rdb.engine.table.Lobs;
...
public class Table extends View {
	...
	public FileChannel raFile; // file handle of the table
	...
	final private ArrayList<TableStorePage> locksInsert = new ArrayList<TableStorePage>(); // liste
																							// der
	// LOCK_INSERT
	final private HashMap<SsConnection, SsConnection> serializeConnections = new HashMap<SsConnection, SsConnection>();
	public final IndexDescriptions indexes;
	public final ForeignKeys references;

	/**
	 * Constructor for read existing tables.
	 */
	Table(Database database, SsConnection con, String name, FileChannel raFile, long offset, int tableFormatVersion)
			throws Exception {
	 ...
	 */
	public Table(Database database, SsConnection con, String name, Columns columns, IndexDescriptions indexes,
	 ...
	 */
	public Table(Database database, SsConnection con, String name, Columns columns, IndexDescriptions existIndexes,
	 ...
	 *
	 */
	public void drop(SsConnection con) throws Exception {
	...
	public StoreImpl getStore(SsConnection con, long filePos, int pageOperation) throws Exception {
	...
	public StoreImpl getStore(TableStorePage storePage, int pageOperation) throws Exception {
	...
	public StoreImpl getStoreInsert(SsConnection con) throws Exception {
	 ...
	 */
	public StoreImpl getStoreTemp(SsConnection con) throws Exception {
	...
	public StoreImpl getLobStore(SsConnection con, long filePos, int pageOperation) throws Exception {
		if (lobs == null) {
			lobs = new Lobs(this);
		}
	 ...
	 */
	public List<StorePageLink> getInserts(SsConnection con) {
		synchronized (locks) {
			ArrayList<StorePageLink> inserts = new ArrayList<StorePageLink>();
	 ...
	 *             if a timeout occurs
	 */
	public final TableStorePage requestLock(SsConnection con, int pageOperation, long page) throws Exception {
		synchronized (locks) {
			if (raFile == null) {
		...
		synchronized (locks) {
			...
			case SQLTokenizer.INSERT: {
				...
				if (serializeConnections.size() > 1)
					return null;
				if (serializeConnections.size() == 1 && serializeConnections.get(con) == null)
					return null;
	 ...
	 */
	public TableStorePage requestWriteLock(SsConnection con, TableStorePage readlock) throws SQLException {
