...
import org.yf.smallsql.rdb.engine.Store;
...
public class StoreImpl extends Store {
	...
	private static final int DEFAULT_PAGE_SIZE = 1024 * 8; // 8 Kb
	 ...
	 */
	public static StoreImpl createStore(Table table, StorePage storePage, int type, long filePos) throws SQLException {
		try {
	 ...
	 */
	public static StoreImpl recreateStore(Table table, StorePage storePage, int type) throws Exception {
	 ...
	 * @throws SQLException
	 */
	public long writeFinsh(SsConnection con) throws SQLException {
		switch (type) {
		case SQLTokenizer.LONGVARBINARY:
		case SQLTokenizer.INSERT:
		case SQLTokenizer.CREATE:
	 ...
	 */
	public final void createWriteLock() throws SQLException {
		TableStorePage storePageWrite = table.requestWriteLock(((TableStorePage) storePage).con,
	 ...
	 */
	public void updateFinsh(SsConnection con, StoreImpl newData) throws SQLException {
		...
		if (newData.offset <= sizePhysical || filePos == -1) {
			...
			if (updatePointer == null) {
				...
				// UPDATE_POINTER
				((TableStorePage) newData.storePage).lockType = View.LOCK_INSERT;
	...
	public int getCurrentOffsetInPage() {
		return offset;
	}

	public void setCurrentOffsetInPage(int newOffset) {
	...
	public int readInt() {
		return ((page[offset++]) << 24) | ((page[offset++] & 0xFF) << 16) | ((page[offset++] & 0xFF) << 8)
				| ((page[offset++] & 0xFF));
	}
	...
	public long readLong() {
		...
		return ((long) (page[offset++]) << 56) | ((long) (page[offset++] & 0xFF) << 48)
				| ((long) (page[offset++] & 0xFF) << 40) | ((long) (page[offset++] & 0xFF) << 32)
				| ((long) (page[offset++] & 0xFF) << 24) | ((page[offset++] & 0xFF) << 16)
	...
	public void writeString(String strDaten) throws SQLException {
	...
	public void writeBytes(byte[] daten, int off, int length) {
	...
	void writeLongBinary(byte[] daten) throws Exception {
		StoreImpl store = table.getLobStore(((TableStorePage) storePage).con, daten.length + 4,
	...
	public void writeChars(char[] daten) {
		int length = daten.length;
	...
	void writeLongString(String daten) throws Exception {
		...
		StoreImpl store = table.getLobStore(((TableStorePage) storePage).con, chars.length * 2L + 4,
				SQLTokenizer.LONGVARBINARY);
	...
	public void writeColumn(Column column) throws Exception {
	 ...
	 * @throws Exception
	 */
	public Column readColumn(int tableFormatVersion) throws Exception {
		Column column = new Column();
		column.setFlag(readByte());
		column.setName(readString());
		column.setDataType(readShort());
		int precision;
		if (tableFormatVersion == View.TABLE_VIEW_OLD_VERSION)
			precision = readByte();
		else
			precision = readInt();
	...
	public void copyValueFrom(StoreImpl store, int valueOffset, int length) {
	 ...
	 */
	public void writeExpression(Expression expr, Column column) throws Exception {
		...
		case SQLTokenizer.UNIQUEIDENTIFIER:
			...
			case SQLTokenizer.BLOB:
				...
				if (bytes.length != 16)
					throw SmallSQLException.create(Language.BYTEARR_INVALID_SIZE, String.valueOf(bytes.length));
				writeBytes(bytes);
			default:
	...
	@Override
	public boolean isNull(int valueOffset) {
	 ...
	 */
	@Override
	public boolean getBoolean(int valueOffset, int dataType) throws Exception {
		...
		default:
			throw SmallSQLException.create(Language.VALUE_CANT_CONVERT,
					new Object[] { SQLTokenizer.getKeyWord(dataType), "BOOLEAN" });
		}
	}
	 ...
	 */
	@Override
	public int getInt(int valueOffset, int dataType) throws Exception {
		this.offset = valueOffset;
		if (readBoolean())
			return 0;
		switch (dataType) {
	 ...
	 */
	@Override
	public long getLong(int valueOffset, int dataType) throws Exception {
		this.offset = valueOffset;
		if (readBoolean())
			return 0;
		switch (dataType) {
		...
		default:
			throw SmallSQLException.create(Language.VALUE_CANT_CONVERT,
					new Object[] { SQLTokenizer.getKeyWord(dataType), "BIGINT" });
		}
	}
	 ...
	 */
	@Override
	public float getFloat(int valueOffset, int dataType) throws Exception {
		this.offset = valueOffset;
		if (readBoolean())
			return 0;
		switch (dataType) {
	 ...
	 */
	@Override
	public double getDouble(int valueOffset, int dataType) throws Exception {
		this.offset = valueOffset;
		if (readBoolean())
			return 0;
		switch (dataType) {
		...
		default:
			throw SmallSQLException.create(Language.VALUE_CANT_CONVERT,
					new Object[] { SQLTokenizer.getKeyWord(dataType), "NUMERIC" });
		}
	}

	// is used for faster calculation
	@Override
	public long getMoney(int valueOffset, int dataType) throws Exception {
		this.offset = valueOffset;
		if (readBoolean())
			return 0;
		switch (dataType) {
		...
		case SQLTokenizer.SMALLDATETIME:
			throw SmallSQLException.create(Language.VALUE_CANT_CONVERT,
					new Object[] { SQLTokenizer.getKeyWord(dataType), "MONEY" });
		default:
			throw new Error();
		}
	}
	 ...
	 */
	@Override
	public MutableNumeric getNumeric(int valueOffset, int dataType) throws Exception {
		this.offset = valueOffset;
		if (readBoolean())
			return null;
		switch (dataType) {
	 ...
	 */
	@Override
	public Object getObject(int valueOffset, int dataType) throws Exception {
		this.offset = valueOffset;
		if (readBoolean())
			return null;
		switch (dataType) {
	 ...
	 */
	@Override
	public String getString(int valueOffset, int dataType) throws Exception {
		this.offset = valueOffset;
		if (readBoolean())
			return null;
		switch (dataType) {
	 ...
	 */
	@Override
	public byte[] getBytes(int valueOffset, int dataType) throws Exception {
		this.offset = valueOffset;
		if (readBoolean())
			return null;
		switch (dataType) {
