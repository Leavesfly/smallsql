...
import org.yf.smallsql.rdb.sql.datatype.Identity;
import org.yf.smallsql.rdb.sql.datatype.Money;
import org.yf.smallsql.rdb.sql.datatype.MutableDouble;
import org.yf.smallsql.rdb.sql.datatype.MutableFloat;
import org.yf.smallsql.rdb.sql.datatype.MutableInteger;
import org.yf.smallsql.rdb.sql.datatype.MutableLong;
...
public class ExpressionValue extends Expression {
	 ...
	 */
	public ExpressionValue(int type) {
		super(type);
		switch (type) {
		case GROUP_BY:
		case SUM:
	 ...
	 */
	public ExpressionValue(Object value, int dataType) {
		super(VALUE);
		this.value = value;
	 ...
	 */
	public void accumulate(Expression expr) throws Exception {
		int type = getType();
		if (type != GROUP_BY)
		...
		case SUM:
			if (isEmpty()) {
				...
				case SQLTokenizer.MONEY:
					...
					break;
				default:
					throw SmallSQLException.create(Language.UNSUPPORTED_TYPE_SUM, SQLTokenizer.getKeyWord(dataType));
				}
			break;
		case MAX:
			if (value == null) {
		...
		case MIN:
			if (value == null) {
				...
				case SQLTokenizer.LONGVARCHAR:
					String str = expr.getString();
					if (String.CASE_INSENSITIVE_ORDER.compare((String) value, str) > 0) // cast
																						// needed
																						// for
																						// Compiler
																						// 1.5
	...
	public final void set(Object value, int _dataType, int length) throws SQLException {
		set(value, _dataType);
		this.length = length;
	}
	...
	public final void set(ExpressionValue val) {
	...
	public static boolean getBoolean(Object obj, int dataType) throws Exception {
		if (obj == null)
			return false;
		switch (dataType) {
	...
	public static int getInt(Object obj, int dataType) throws Exception {
		if (obj == null)
			return 0;
		switch (dataType) {
		case SQLTokenizer.BIT:
	...
	public static long getLong(Object obj, int dataType) throws Exception {
		if (obj == null)
			return 0;
		switch (dataType) {
		case SQLTokenizer.BIT:
	...
	public static float getFloat(Object obj, int dataType) throws Exception {
		if (obj == null)
			return 0;
		switch (dataType) {
		case SQLTokenizer.BIT:
	...
	public static double getDouble(Object obj, int dataType) throws Exception {
		if (obj == null)
			return 0;
		switch (dataType) {
		case SQLTokenizer.BIT:
	...
	public static long getMoney(Object obj, int dataType) throws Exception {
		if (obj == null)
			return 0;
		switch (dataType) {
		case SQLTokenizer.BIT:
	...
	public MutableNumeric getNumeric() {
		return getNumeric(getObject(), dataType);
	}

	public static MutableNumeric getNumeric(Object obj, int dataType) {
		if (obj == null)
			return null;
		switch (dataType) {
		case SQLTokenizer.BIT:
	...
	public static byte[] getBytes(Object obj, int dataType) throws Exception {
		if (obj == null)
			return null;
		switch (dataType) {
		...
		default:
			throw createUnsupportedConversion(dataType, obj, SQLTokenizer.VARBINARY);
		}
	}
	...
	static SQLException createUnsupportedConversion(int fromDataType, Object obj, int toDataType) {
		Object[] params = { SQLTokenizer.getKeyWord(fromDataType), obj, SQLTokenizer.getKeyWord(toDataType) };
