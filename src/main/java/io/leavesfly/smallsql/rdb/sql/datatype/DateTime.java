 ...
 */
package io.leavesfly.smallsql.rdb.sql.datatype;
...
public final class DateTime implements Mutable {
	...
	public DateTime(long time, int dataType) {
		switch (dataType) {
	...
	public static long calcMillis(Details details) {
		return calcMillis(details.year, details.month, details.day, details.hour, details.minute, details.second,
				details.millis);
	}

	static long calcMillis(int year, int month, final int day, final int hour, final int minute, final int second,
			final int millis) {
		long result = millis;
		result += second * 1000;
		result += minute * 60000;
	...
	public static long parse(java.util.Date date) {
		long t = date.getTime();
		return removeDateTimeOffset(t);
	}

	public static DateTime valueOf(java.util.Date date) {
		if (date == null)
			return null;
		int type;
	...
	public static DateTime valueOf(java.sql.Date date) {
		if (date == null)
			return null;
		return new DateTime(parse(date), SQLTokenizer.DATE);
	}

	public static DateTime valueOf(java.sql.Time date) {
		if (date == null)
			return null;
		return new DateTime(parse(date), SQLTokenizer.TIME);
	}

	public static DateTime valueOf(java.sql.Timestamp date) {
		if (date == null)
			return null;
	 ...
	 */
	public static DateTime valueOf(String date, int dataType) throws SQLException {
		if (date == null)
			return null;
		return new DateTime(parse(date), dataType);
	}

	public static long parse(final String datetime) throws SQLException {
		try {
	...
	public final static long addDateTimeOffset(long datetime, TimeZone timezone) {
