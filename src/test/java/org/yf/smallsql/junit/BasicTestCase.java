...
public class BasicTestCase extends TestCase {
	...
	protected void dropTable(Connection con, String name) throws SQLException {
		try {
	...
	public void dropView(Connection con, String name) {
		try {
			Statement st = con.createStatement();
	...
	public void assertRSMetaData(ResultSet rs, String[] colNames, int[] types) throws Exception {
		...
		for (int i = 1; i <= count; i++) {
			...
			case Types.VARCHAR:
				assertTrue(
						"Wrong Precision (" + rm.getColumnTypeName(i) + ") for Column " + i + ": " + rm.getPrecision(i),
						rm.getPrecision(i) > 0);
				break;
			case Types.INTEGER:
	...
	protected void assertRowCount(int sollCount, String sql) throws Exception {
	...
	protected void assertRowCount(int sollCount, ResultSet rs) throws Exception {
	 ...
	 */
	protected void assertEqualsRsValue(Object obj, String sql) throws Exception {
		Connection con = AllTests.getConnection();
		Statement st = con.createStatement();
		ResultSet rs = st.executeQuery(sql);
	...
	public void assertEqualsRsValue(Object obj, ResultSet rs, boolean needTrim) throws Exception {
		...
		if (obj != null) {
			...
			if (gotClass.equals(java.sql.Blob.class)) {
				assertTrue("ClassName assignable: " + className + "<->" + objClassName,
				...
				assertTrue("ClassName assignable: " + className + "<->" + objClassName, objClass.equals(String.class));
				...
				assertTrue("ClassName assignable: " + className + "<->" + objClassName,
						gotClass.isAssignableFrom(objClass));
				assertTrue("DisplaySize to small " + expectedLen + "<" + foundStr.length() + " (" + foundStr + ")",
						expectedLen >= foundStr.length());
			}
		}
	}

	protected void assertSQLException(String sqlstate, int vendorCode, SQLException ex) {
	 ...
	 */
	public static String getMonth3L(int ordinal) {
